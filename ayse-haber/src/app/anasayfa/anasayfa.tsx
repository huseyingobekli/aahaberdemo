...
export default function Anasayfa() {
        ...
        if (data?.type === 'FeatureCollection' && Array.isArray(data?.features)) {
          ...
          const items: NewsItem[] = features
            .map((f: any) => ({
              ...
              description: typeof f?.properties?.description === 'string' ? f.properties.description : '',
              content: typeof f?.properties?.content === 'string' ? f.properties.content : undefined,
              contentTop: f?.properties?.contentTop,
              contentBottom: f?.properties?.contentBottom,
              location: typeof f?.properties?.location === 'string' ? f.properties.location : undefined,
              city: typeof f?.properties?.city === 'string' ? f.properties.city : undefined,
              img: typeof f?.properties?.img === 'string' ? f.properties.img : undefined,
              credit: typeof f?.properties?.credit === 'string' ? f.properties.credit : undefined,
            }))
  ...
  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + Math.min(5, newsItems.length)) % Math.min(5, newsItems.length))
  }
          ...
          <div className="anasayfa-slider">
            <div className="slider-container">
              {sliderItems.map((item, index) => (
                ...
                >
                  <Link href={`/haber/${index}`} className="slider-link">
                    <img 
              ...
              <div className="bottom-cards-grid">
                {bottomItems.map((item, index) => (
                  <div key={index} className="bottom-card">
                    <Link href={`/haber/${index + 5}`} className="bottom-card-link">
            ...
            <div className="sidebar-items">
              {newsItems.slice(0, 8).map((item, index) => (
                <div key={index} className="sidebar-item">
                  <Link href={`/haber/${index}`} className="sidebar-link">
